      ...
      <div class="container">
        <div class="order-box">
          ...
          <div class="btn-group">
            ...
            <a href="javascript:;" class="btn btn-large" @click="orderSubmit">结算</a>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      count: 0,         //要购买的商品总件数
      showDelModal:false, //删除地址对话框
      showEditModal:false, //新增/编辑地址对话框
      checkedItem:{},        //选中的商品对象
      userAction: '',       //用户行为 0：新增 1：编辑 2：删除
      checkIndex:0        //当前收货地址选中索引,默认0
    }
  },
  mounted() {
    this.getAddressList();
  ...
  methods: {
    ...
    openAddressModal() {
      this.userAction = 0;
      this.checkedItem = {}; //不需要传item,下一步打开对话框,里面v-model="checkedItem.receiverName"相等于给checkedItem设置了receiverName属性
    ...
    submitAddress() {
      // checkedItem是传过来的item,userAction用来区别是新增/修改/删除
      let {checkedItem, userAction} = this;
      let method, url, params = {};
      if(userAction == 0) {
        method = 'post', url = '/shippings';
      } else if(userAction == 1) {
         method = 'put',url = `/shippings/${checkedItem.id}`; //checkedItem.id是地址列表的id
      } else {
         method = 'delete',url = `/shippings/${checkedItem.id}`;
      }
      if(userAction == 0 || userAction == 1) {
        // 从传过来的checkedItem里解构出需要的参数
    ...
    //结算 到支付页面
    orderSubmit() {
      //item 收获地址索引，如果收获地址索引对应的地址被删除，error
      ...
      this.axios.post('/orders', {
        ...
        this.$router.push({
          path: '/order/pay',
          query: {
            orderNo: res.orderNo   //把接口返回的订单号作为参数传到订单支付页面
          }
        })
      }))
    }
